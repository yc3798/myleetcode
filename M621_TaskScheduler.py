...
class Solution:
	def leastInterval(self, tasks, n: int) -> int:
		...
		rank = sorted(list(counttasks.items()), key = lambda i : i[1], reverse = True)
		rank = [i[0] for i in rank]  # serve as a Queue
		
		time = 0
		timelog = {i:0 for i in counttasks} # create a dictionary to log time, timelog = {}
		...
		while rank:
			...
			if timelog[task] != 0 and time - timelog[task] < n: # need IDLE 
				process.extend(["IDLE" for i in range(n - (time - timelog[task]))])
				time += (n - (time - timelog[task]))
			...
			if counttasks[task] > 0: 
				if n >= len(rank): # send task to back of rank list
					rank.append(task)
				else: # insert task into rank at index t where counttasks[t] < counttask[task]
					t = n 
					while t < len(rank) and counttasks[rank[t]] > counttasks[task]: # find the write index to insert
						t += 1
					rank.insert(t, task)
