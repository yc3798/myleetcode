...
class Solution:
	def leastInterval(self, tasks, n: int) -> int:
		...
		seq = [i[0] for i in rank] # [A, B, C ...]
		rank = [i[0] for i in rank]  # serve as a PQ
		
		# create a dictionary to log time, timelog = {}
		time = 0
		timelog = {i:0 for i in counttasks}
		...
		while rank:
			...
			if counttasks[task] > 0: #send task to back of rank list
				if len(rank) >= n + seq.index(task) + 1:
					rank.insert(n + seq.index(task),task)
				else:
					rank.append(task)
			else:
				del counttasks[task]
